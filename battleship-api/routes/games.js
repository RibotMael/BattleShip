...
router.get("/public", async (req, res) => {
  ...
  `);
    ...
    res
      ...
      .json({
        success: false,
        message: "Erreur lors de la récupération des parties publiques.",
      });
  }
});
...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    const rawLang = versionRows.length
      ? versionRows[0].name.toLowerCase()
      : "fr";
    let langKey;
    if (rawLang.startsWith("fr")) langKey = "fr";
    else if (
...
router.post("/join", async (req, res) => {
  ...
  try {
    ...
    if (!games.length)
      return res
        .status(404)
        .json({ success: false, message: "Partie introuvable" });
    ...
    if (existing.length)
      return res
        .status(400)
        .json({ success: false, message: "Joueur déjà dans la partie" });

    await db.execute(
      "INSERT INTO game_players (id_game, id_player) VALUES (?, ?)",
      [gameId, playerId]
    );
    ...
    res
      ...
      .json({
        success: false,
        message: "Erreur serveur lors de la jointure.",
      });
  }
});
...
router.post("/kick", async (req, res) => {
  ...
  try {
    ...
    await db.execute(
      "DELETE FROM game_players WHERE id_game = ? AND id_player = ?",
    ...
    res
      .status(500)
      .json({ success: false, message: "Erreur lors de l'expulsion" });
  }
});
...
router.post("/leave", async (req, res) => {
  ...
  try {
    ...
    if (isHost) {
      await db.execute(
        "DELETE FROM game_players WHERE id_game = ?",
        [gameId]
      );
      await db.execute("DELETE FROM games WHERE id_Game = ?", [gameId]);
      return res.json({
        success: true,
        message: "Partie supprimée par l'hôte",
      });
    } else {
      ...
      if (game.status === "in_progress") {
        const [stillInGame] = await db.execute(
          "SELECT COUNT(*) as count FROM game_players WHERE id_game = ?",
          [gameId]
        );
        let requiredPlayers;
        ...
        if (stillInGame[0].count < requiredPlayers) {
          await db.execute(
            "UPDATE games SET status = 'preparation' WHERE id_Game = ?",
            [gameId]
          );
        }
      }
    ...
    res
      ...
      .json({
        success: false,
        message: "Erreur serveur lors du départ du joueur",
      });
  }
});
  ...
  try {
    ...
    if (Number(game.id_creator) !== playerId)
      return res
        ...
        .json({
          success: false,
          message: "Seul l'hôte peut supprimer la partie",
        });
...
router.get("/:gameId/board", async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        success: false,
        message: "ID de partie ou de joueur invalide.",
      });
  }
  ...
  try {
    ...
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Format de plateau invalide en base.",
        });
    }
...
router.post("/shoot", async (req, res) => {
  ...
  if (x < 0 || x > 9 || y < 0 || y > 9) {
    return res
      .status(400)
      .json({ success: false, message: "Coordonnées invalides" });
  }
...
router.get("/:gameId/shots", async (req, res) => {
  ...
  try {
    ...
    const incomingShots = enhancedShots.filter(
      (s) => s.target_id === playerId
    );
